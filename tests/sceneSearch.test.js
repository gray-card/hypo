...
const edge = (rk, from, to, id, label) => ({
  uri: `at://did/scene.edge/${rk}`,
  value: { scene: G, type: { id, label }, from: `at://did/scene.node/${from}`, to: `at://did/scene.node/${to}` },
});
...
const FIXTURE = {
  ...
  sceneEdges: [edge("e1", "dog", "tree", "left-of", "left of"), edge("e2", "person", "bike", "riding", "riding")],
};
...
describe("parseQuery", () => {
  ...
  it("detects a non-spatial relation present in the corpus", () => {
    expect(parseQuery("person riding bicycle", forms).triple).toEqual({
      subj: "person",
      rel: "riding",
      obj: "bicycle",
    });
...
describe("searchScenes — regressions caught in adversarial review", () => {
  ...
  const SG = "at://did/scene.graph/gS",
    SP = "at://did/social.grain.photo/pS";
  ...
  const F = {
    ...
    sceneEdges: [
      {
        uri: "at://did/scene.edge/e",
        value: {
          scene: SG,
          type: { id: "adjacent-to", label: "next to" },
          from: "at://did/scene.node/d",
          to: "at://did/scene.node/t",
        },
      },
    ],
  };
...
describe("parseQueryToIR (shared query IR from the heuristic)", () => {
  ...
  it("splits AND (comma/and) and OR", () => {
    expect(parseQueryToIR("dog and tree")).toMatchObject({
      match: "all",
      clauses: [
        { kind: "object", concept: "dog" },
        { kind: "object", concept: "tree" },
      ],
    });
    expect(parseQueryToIR("dog or cat")).toMatchObject({
      match: "any",
      clauses: [{ concept: "dog" }, { concept: "cat" }],
    });
  });
  it("keeps a multi-word object whole, but splits known labels via objectForms", () => {
    ...
    expect(parseQueryToIR("dog tree", { objectForms: objs }).clauses).toEqual([
      { kind: "object", concept: "dog" },
      { kind: "object", concept: "tree" },
    ]);
  });
  ...
  it("detects relations with wildcard subject or object", () => {
    expect(parseQueryToIR("near a tree", { relationForms: forms }).clauses).toEqual([
      { kind: "relation", subject: null, relation: "near", object: "tree" },
    ]);
    expect(parseQueryToIR("dog on top of", { relationForms: forms }).clauses).toEqual([
      { kind: "relation", subject: "dog", relation: "on top of", object: null },
    ]);
  });
});
...
describe("validateIR (repair external/LLM output)", () => {
  it("strips a leaked QID and drops empty clauses", () => {
    ...
    expect(validateIR({ clauses: [{ kind: "object", concept: "dog" }] })).toEqual({
      match: "all",
      clauses: [{ kind: "object", concept: "dog" }],
    });
  });
  it("forbids negate + minCount together (negate wins)", () => {
    expect(
      validateIR({ match: "all", clauses: [{ kind: "object", concept: "dog", negate: true, minCount: 3 }] }),
...
describe("query parser fixes (adversarial review of the parsers)", () => {
  ...
  it("mixed AND/OR keeps precedence: (dog AND cat) OR bird", async () => {
    ...
    expect(ir.clauses[0]).toEqual({
      kind: "group",
      match: "all",
      clauses: [
        { kind: "object", concept: "dog" },
        { kind: "object", concept: "cat" },
      ],
    });
  ...
  it("leading-hyphen negation works for single and multi-term queries", () => {
    ...
    expect(parseQueryToIR("dog -cat").clauses).toEqual([
      { kind: "object", concept: "dog" },
      { kind: "object", concept: "cat", negate: true },
    ]);
  ...
  it("segmentObjects recovers a known object after an unknown token", () => {
    expect(parseQueryToIR("dog brown tree", { objectForms: objs }).clauses).toEqual([
      { kind: "object", concept: "dog" },
      { kind: "object", concept: "brown" },
      { kind: "object", concept: "tree" },
    ]);
  });
...
describe("graded scoring + fusion + banding", () => {
  ...
  it("scoreIR grades exact 1.0 > hierarchy member 0.7 > no-match 0", () => {
    expect(
      scoreIR(recDog, { match: "all", clauses: [{ kind: "object", concept: "dog" }] }, r({ dog: "Q144" }), anc),
    ).toBe(1);
    expect(
      scoreIR(recDog, { match: "all", clauses: [{ kind: "object", concept: "animal" }] }, r({ animal: "Q729" }), anc),
    ).toBeCloseTo(0.7, 5);
    expect(
      scoreIR(recDog, { match: "all", clauses: [{ kind: "object", concept: "cat" }] }, r({ cat: "Q146" }), anc),
    ).toBe(0);
  });
  it("2-of-2 outranks 1-of-2 via the mean aggregation", () => {
    const two = scoreIR(
      recDog,
      {
        match: "all",
        clauses: [
          { kind: "object", concept: "animal" },
          { kind: "object", concept: "dog" },
        ],
      },
      r({ animal: "Q729", dog: "Q144" }),
      anc,
    );
    const one = scoreIR(
      recDog,
  ...
  it("LLM rerank enters the fusion when provided", async () => {
    const idx = buildSceneIndex(FIXTURE);
    const seen = [];
    const llmRerank = async (q, uris) => {
      seen.push(...uris);
      return new Map(uris.map((u) => [u, 1]));
    };
...
describe("negation over-exclusion fix (edge-less / empty graphs)", () => {
  ...
  const F = {
    scenes: [{ uri: "g", value: { subject: "pc" } }],
    sceneNodes: [{ uri: "n", value: { scene: "g", type: { id: "Q146", label: "cat" } } }],
    sceneEdges: [],
  };
  const idx = buildSceneIndex(F);
  ...
  const d = {
    resolveTerm: async (t) => (ent[t.toLowerCase()] ? { qid: ent[t.toLowerCase()] } : null),
...
describe("rankScenes — progressive render + deferred LLM rerank", () => {
  const idx = buildSceneIndex({
    scenes: [
      { uri: "g1", value: { subject: "p1" } },
      { uri: "g2", value: { subject: "p2" } },
    ],
    sceneNodes: [
  ...
  it("paints the fast result via onPartial BEFORE the LLM reranker runs", async () => {
    ...
    const onPartial = (r) => {
      partial = r;
      expect(llmStarted).toBe(false);
    }; // fast result precedes the LLM
  ...
  it("returns the fast result directly and never calls onPartial when no reranker is given", async () => {
    let called = false;
    const r = await rankScenes(idx, "dog", {
      ...dep,
      onPartial: () => {
        called = true;
      },
