...
describe("instanceImageUrl", () => {
  ...
  it("falls back to the type's Wikidata stock image (camera via type ref)", async () => {
    ...
    const store = storeWithType("at://t/cam", {
      make: "Leica",
      model: "M6",
      links: { externalIds: [{ scheme: "wikidata", value: "Q-gi-cam" }] },
    });
  ...
  it("resolves film rolls through their stock reference", async () => {
    ...
    const store = storeWithType("at://t/film", {
      brand: "Kodak",
      name: "Portra 400",
      links: { externalIds: [{ scheme: "wikidata", value: "Q-gi-film" }] },
    });
...
describe("gearThumb", () => {
  it("returns a .type-thumb element and paints the resolved image on refresh", async () => {
    ...
    const store = storeWithType("at://t/cam", {
      make: "Leica",
      model: "M6",
      links: { externalIds: [{ scheme: "wikidata", value: "Q-gi-thumb" }] },
    });
