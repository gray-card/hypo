...
describe("writeSceneGraph", () => {
  ...
  it("skips edges whose nodes were dropped", async () => {
    ...
    const analysis = {
      nodes: [{ key: "o1", type: { id: "cat", label: "cat" }, label: "", box: null }],
      edges: [{ from: "o1", to: "missing", type: { id: "x", label: "x" } }],
    };
...
describe("Claude provider network contract", () => {
  ...
  it("validateConfig requires a key and pings /models with the browser-access header", async () => {
    ...
    await expect(validateConfig({ provider: "claude", apiKey: "sk-ant-abc", model: "claude-opus-4-8" })).resolves.toBe(
      true,
    );
  ...
  it("analyze posts an image + JSON-schema request and normalizes the structured reply", async () => {
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            ...
            objects: [
              { key: "o1", type: "person" },
              { key: "o2", type: "bicycle", box: { x: 0.1, y: 0.1, w: 0.2, h: 0.3 } },
            ],
    ...
    expect(content.find((b) => b.type === "image").source).toMatchObject({
      type: "base64",
  ...
  it("analyze raises on an API error and on a refusal", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => jsonResponse(400, { error: { message: "too big" } })),
    );
    await expect(
      PROVIDERS.claude.analyze({ apiKey: "k", model: "m" }, { base64: "x", mediaType: "image/png" }),
    ).rejects.toThrow(/too big/);

    vi.stubGlobal(
      "fetch",
      vi.fn(async () => jsonResponse(200, { stop_reason: "refusal", content: [] })),
    );
    await expect(
      PROVIDERS.claude.analyze({ apiKey: "k", model: "m" }, { base64: "x", mediaType: "image/png" }),
...
describe("Gemini provider network contract", () => {
  ...
  it("validateConfig routes to Gemini and pings /models with the x-goog-api-key header", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await expect(validateConfig({ provider: "gemini", apiKey: "AIzaXYZ", model: "gemini-flash-latest" })).resolves.toBe(
      true,
    );
  ...
  it("surfaces a rejected key (400/403) with the API message", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => jsonResponse(400, { error: { message: "API key not valid" } })),
    );
            ...
            parts: [
              {
                text: JSON.stringify({
                  ...
                  objects: [
                    { key: "o1", type: "boat", box: { x: 0.2, y: 0.3, w: 0.4, h: 0.2 } },
                    { key: "o2", type: "water" },
                  ],
  ...
  it("raises on an API error, a prompt block, and an unusable finishReason", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => jsonResponse(200, { candidates: [{ finishReason: "SAFETY", content: { parts: [] } }] })),
    );
    await expect(
      PROVIDERS.gemini.analyze({ apiKey: "k", model: "m" }, { base64: "x", mediaType: "image/jpeg" }),
...
describe("resolveModel auto-heals retired model ids", () => {
  ...
  it("analyze sends the default when the stored model was retired", async () => {
    const reply = {
      candidates: [
        { finishReason: "STOP", content: { parts: [{ text: '{"altText":"x","objects":[],"relations":[]}' }] } },
      ],
    };
    const fetchMock = vi.fn(async () => jsonResponse(200, reply));
    vi.stubGlobal("fetch", fetchMock);
    await PROVIDERS.gemini.analyze(
      { apiKey: "k", model: "gemini-2.5-flash" },
      { base64: "x", mediaType: "image/jpeg" },
    );
...
describe("clearSceneGraph", () => {
        ...
        atproto: {
          repo: {
            listRecords: async ({ collection }) => ({
              data: {
                records: (byCollection[collection] || []).map((r) => ({ uri: r.uri, cid: "c", value: r.value })),
              },
            }),
  ...
  it("deletes every graph bound to the photo plus its nodes/edges/regions, and nothing else", async () => {
    const g1 = uri(NS.scene.graph, "g1"),
      g2 = uri(NS.scene.graph, "g2"),
      gOther = uri(NS.scene.graph, "g3");
...
describe("provider.describe (alt text only, no object detection)", () => {
  ...
  it("Claude: posts a plain completion (no JSON schema) and returns cleaned alt text", async () => {
    const fetchMock = vi.fn(async () =>
      jsonResponse(200, { content: [{ type: "text", text: '  "A dog on a porch."\n' }] }),
    );
    vi.stubGlobal("fetch", fetchMock);
    const alt = await PROVIDERS.claude.describe(
  ...
  it("Gemini: posts a text request (no responseMimeType json) and returns alt text", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const alt = await PROVIDERS.gemini.describe(
