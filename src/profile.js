...
export async function resolvePds(did) {
  ...
  if (did.startsWith("did:plc:")) doc = await json(`https://plc.directory/${did}`);
    ...
    doc = await json(`https://${did.slice(8).replace(/:/g, "/")}/.well-known/did.json`);
...
export async function listPublic(pds, did, collection) {
  try {
    return await new PublicRepoClient(pds).listAll({ repo: did, collection, limit: 100 });
  } catch {
    return [];
  }
}
...
export async function loadSetup(handle) {
  ...
  await Promise.all([
    ...catalogKinds.map(async (k) => {
      catalog[k] = await grab(NS.catalog[k]);
      for (const it of catalog[k]) byUri.set(it.uri, { layer: "catalog", kind: k, item: it });
    }),
    ...instanceKinds.map(async (k) => {
      instance[k] = await grab(NS.instance[k]);
      for (const it of instance[k]) byUri.set(it.uri, { layer: "instance", kind: k, item: it });
    }),
  ]);
  const [
    templates,
...
let gcDisk = {};
try {
  gcDisk = JSON.parse(localStorage.getItem(GC_KEY) || "{}");
} catch {
  /* ignore */
}
  ...
  try {
    do {
      ...
      for (const f of j.follows || [])
        out.push({ did: f.did, handle: f.handle, displayName: f.displayName, avatar: f.avatar });
      cursor = j.cursor;
  ...
  for (let i = 0; i < uniq.length; i += 25) {
    try {
      ...
      for (const p of j.profiles || [])
        out.push({ did: p.did, handle: p.handle, displayName: p.displayName, avatar: p.avatar });
    } catch {
      /* skip batch */
    }
  }
...
export async function getGrainFollows(did) {
  ...
  try {
    ...
    const records = await new PublicRepoClient(pds).listAll({
      repo: did,
      collection: "social.grain.graph.follow",
      limit: 100,
    });
    for (const record of records) if (record.value?.subject) subjects.push(record.value.subject);
  } catch {
    /* no grain follows / offline */
  }
...
export function hasGraycard(did) {
  ...
  const save = (rec) => {
    gcDisk[did] = rec;
    try {
      localStorage.setItem(GC_KEY, JSON.stringify(gcDisk));
    } catch {
      /* quota */
    }
  };
  const p = (async () => {
    try {
      const v = await withTimeout(
        (async () => {
          const pds = await resolvePds(did);
          const description = await new PublicRepoClient(pds).describe({ repo: did });
          return description.collections.some((collection) => collection.startsWith("app.graycard."));
        })(),
        GC_TIMEOUT_MS,
      );
      save({ v, t: Date.now() });
