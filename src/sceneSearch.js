...
export function normalizeString(s) {
  s = String(s ?? "")
    .normalize("NFKD")
    .replace(/[̀-ͯ]/g, ""); // 1-3: NFKD + drop combining marks
...
export function stem(tok) {
  ...
  if (tok.endsWith("ies")) {
    const st = tok.slice(0, -3) + "y";
    return st.length >= 3 ? st : tok;
  } // cities -> city
  if (/(?:sses|xes|zzes|ches|shes)$/.test(tok)) {
    const st = tok.slice(0, -2);
    return st.length >= 3 ? st : tok;
  } // glasses -> glass
  if (tok.endsWith("s") && !/(?:ss|us|is)$/.test(tok)) {
    const st = tok.slice(0, -1);
    return st.length >= 3 ? st : tok;
  } // dogs -> dog
  return tok;
}
...
export function buildSceneIndex({ scenes = [], sceneNodes = [], sceneEdges = [] } = {}) {
  ...
  for (const [g, photo] of photoByGraph) {
    ...
    for (const e of edgesByGraph.get(g) || [])
      rec.edges.push({ ...e, fromNode: nodeById.get(e.from) || null, toNode: nodeById.get(e.to) || null });
...
export function bm25Search(index, terms, { k1 = 1.5, b = 0.75, idf } = {}) {
  ...
  for (const term of new Set(terms)) {
    ...
    for (const [uri, tf] of p) {
      ...
      scores.set(
        uri,
        (scores.get(uri) || 0) + (termIdf * (tf * (k1 + 1))) / (tf + k1 * (1 - b + b * (dl / (index.avgdl || 1)))),
      );
    }
  }
  return scores;
...
const known = (phrase, relationForms) => {
  const p = norm(phrase);
  return resolveRelation(p).id != null || relationForms.has(p);
};
...
function segmentObjects(np, objectForms) {
  ...
  while (i < w.length) {
    let hit = null;
    for (let j = Math.min(w.length, i + 5); j > i; j--) {
      const cand = w.slice(i, j).join(" ");
      if (objectForms.has(cand)) {
        hit = { cand, len: j - i };
        break;
      }
    }
    if (hit) {
      ...
      buf.push(w[i]);
      i += 1;
    } // buffer unknown words; resume scanning
  }
...
export function parseQueryToIR(text, { relationForms = new Set(), objectForms = new Set() } = {}) {
  ...
  const clauses = groups
    .map((g) => {
      const inner = parseGroup(g);
      return inner.length === 1 ? inner[0] : { kind: "group", match: "all", clauses: inner };
    })
...
export function validateIR(raw) {
  ...
  for (const c of raw.clauses) {
    ...
    if (c.kind === "relation") {
      ...
      if (negate)
        clause.negate = true; // negate wins; drop any minCount
...
function edgeRelationMatches(edge, rel) {
  if (rel.concept && edge.concept) {
    ...
    if (edge.concept === rel.concept)
      return { ok: true, inverse: false, symmetric: RELATION_CONCEPTS[rel.concept]?.symmetric === true };
  ...
  if (rel.text && (norm(edge.relLabel).includes(rel.text) || norm(edge.relId).includes(rel.text)))
    return { ok: true, inverse: false, raw: true };
  return { ok: false };
}
...
export function evaluateIR(rec, ir, resolved, ancestors) {
  ...
  const rs = ir.clauses.map((c) =>
    c.kind === "group"
      ? evaluateIR(rec, c, resolved, ancestors)
      : c.kind === "relation"
        ? evalRelation(c, rec, resolved, ancestors)
...
function objectScore(c, rec, resolved, ancestors) {
  ...
  const C = resolved.get(c.concept) || { text: c.concept };
  const w = rec.nodes
    .map((n) => nodeMatchWeight(n, C, ancestors))
    .filter((x) => x > 0)
    .sort((a, b) => b - a);
...
function clauseScore(c, rec, resolved, ancestors) {
  if (c.kind === "group") {
    const rs = c.clauses.map((x) => clauseScore(x, rec, resolved, ancestors));
    return c.match === "any" ? Math.max(0, ...rs) : rs.length ? rs.reduce((a, b) => a + b, 0) / rs.length : 0;
  }
...
function negationViolated(ir, rec, resolved, ancestors) {
  const bad = (clauses) =>
    clauses.some((c) => {
      ...
      return c.kind === "relation"
        ? evalRelation({ ...c, negate: false }, rec, resolved, ancestors)
        : evalObject({ ...c, negate: false }, rec, resolved, ancestors);
    });
...
function rrf(lists, { k = 60, weights = [] } = {}) {
  ...
  lists.forEach((L, i) => {
    if (!L) return;
    const w = weights[i] ?? 1;
    for (const [uri, rank] of L) fused.set(uri, (fused.get(uri) || 0) + w / (k + rank));
  });
  return fused;
}
...
export async function rankScenes(index, query, deps = {}) {
  ...
  if (llmParse) {
    onStage?.("parse");
    try {
      ir = validateIR(await llmParse(query));
    } catch {
      ir = null;
    }
  }
  ...
  const collect = (cs) => {
    for (const c of cs) {
      if (c.kind === "group") collect(c.clauses);
      else if (c.kind === "relation") {
        for (const L of [c.subject, c.object])
          if (L) {
            labels.add(L);
            if (!c.negate) textLabels.add(L);
          }
      } else if (c.concept) {
        labels.add(c.concept);
        if (!c.negate) textLabels.add(c.concept);
      }
    }
  };
  collect(ir.clauses);
  ...
  await Promise.all(
    [...labels].map(async (t) => {
      let hit = null;
      try {
        hit = resolveTerm ? await resolveTerm(t) : null;
      } catch {
        hit = null;
      }
  ...
  try {
    if (ancestorsOf && index.allNodeQids.length) ancestors = await ancestorsOf(index.allNodeQids);
  } catch {
    ancestors = new Map();
  }
  ...
  for (const rec of index.photos.values()) {
    ...
    if (negationViolated(ir, rec, resolved, ancestors)) {
      excluded.add(uri);
      continue;
    } // hard drop
  ...
  const bm25 = textIndex
    ? bm25Search(
        textIndex,
        [...textLabels].flatMap((L) => (tokenizer || tokenize)(L)),
        { idf: captionIdf },
      )
    : new Map();
  ...
  const buildResults = (llm) => {
    const fused = rrf([rankMap(wd), rankMap(bm25), llm.size ? rankMap(llm) : undefined], {
      k: cfg.k,
      weights: [cfg.wWd, cfg.wBm25, cfg.wLlm],
    });
    const match = [],
      near = [],
      tail = [];
  ...
  if (deps.onPartial) {
    try {
      ...
      /* a render error must not abort search */
    }
  }

  let llm = new Map();
  const prelim = rrf([rankMap(wd), rankMap(bm25)], { k: cfg.k, weights: [cfg.wWd, cfg.wBm25] });
  const cands = [...prelim.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 30)
    .map(([uri]) => uri);
  try {
    const r = await llmRerank(query, cands);
    llm = r instanceof Map ? r : new Map(Object.entries(r || {}));
  } catch {
    llm = new Map();
  }
