...
export function openBundleModal(agent, did) {
  ...
  const exportBtn = el(
    "button",
    {
      ...
      onclick: async (e) => {
        ...
        try {
          const b = await exportBundle(agent, did);
          download(`graycard-bundle-${Date.now()}.json`, b);
          toast(`Exported ${b.records.length} records`, "ok");
        } catch (err) {
  ...
  pruneChk.addEventListener("change", () => {
    if (basePlan) renderPlan();
  });
  const pruneToggle = el("label", { class: "inline-check prune-toggle" }, [
    pruneChk,
    "Prune: delete records not in the bundle",
  ]);
  ...
  function renderPlan() {
    ...
    review.append(
      el(
        ...
        `${plan.length} records · ${c.create || 0} new · ${c.update || 0} changed · ${c.unchanged || 0} unchanged` +
          (pruneList.length ? ` · ${c.delete || 0} to delete` : ""),
      ),
    );
    ...
    for (const p of plan) {
      ...
      if (p.status !== "unchanged") {
        row.append(
          el("details", {}, [
            el("summary", { class: "small muted" }, "value"),
            el("pre", { class: "inspect-json" }, JSON.stringify(p.value, null, 2)),
          ]),
        );
      }
      list.append(row);
            ...
            onclick: async (e) => {
              ...
              if (overwrites.length || deletes.length) {
                ...
                if (overwrites.length)
                  parts.push(`overwrite ${overwrites.length} existing record${overwrites.length !== 1 ? "s" : ""}`);
              ...
              const results = await writeBundle(
                agent,
                did,
                plan,
                (it, done, total) => {
              ...
              status.textContent =
                `Wrote ${n("written")}, deleted ${n("deleted")}` +
                (n("conflict") ? `, ${n("conflict")} conflicts` : "") +
  ...
  modal.append(
    ...
    el(
      "p",
      { class: "muted small" },
      "Export every app.graycard.* record in your repo, or load a Gray Card bundle to review a diff and write it to your PDS. Re-importing an export is a no-op — unchanged records are skipped and you're asked before any overwrite.",
    ),
