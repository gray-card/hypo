...
export async function openLocationPicker(initial = null) {
  ...
  return new Promise((resolve) => {
    ...
    const hint = el(
      "p",
      { class: "muted small" },
      "Search for a place, tap the map to drop a pin, drag to fine-tune, or use your current location. Published locations are rounded to a ~5 km area.",
    );
    ...
    const modal = el(
      "div",
      { class: "card modal map-modal", role: "dialog", "aria-modal": "true", "aria-label": "Set location" },
      [
        el("h2", {}, "Set location"),
        hint,
    ...
    useMe.addEventListener("click", () => {
      ...
      useMe.disabled = true;
      useMe.textContent = "Locating…";
      navigator.geolocation.getCurrentPosition(
        (p) => {
          const { longitude, latitude } = p.coords;
          map.flyTo({ center: [longitude, latitude], zoom: 13 });
          place(longitude, latitude);
          useMe.disabled = false;
          useMe.textContent = "Use my location";
        },
        () => {
          useMe.disabled = false;
          useMe.textContent = "Location unavailable";
        },
...
export function locationField(initial) {
  let loc = initial
    ? {
        ...
        ...(initial.placemark ? { placemark: initial.placemark } : {}),
      }
    : null;
  ...
  const btn = el(
    "button",
    {
      ...
      onclick: async () => {
        ...
        loc = r;
        render(); // geoLocation, or null when cleared
      },
    },
    "Set on map",
  );
...
export async function mountHeatmap(node, state, cells, selected, onToggle) {
  ...
  if (state.map) {
    // already built: refresh data only (no re-fit, so unrelated filter changes don't jump the view)
  ...
  const build = () => {
    ...
    map.on("mouseenter", "cells-dot", () => {
      map.getCanvas().style.cursor = "pointer";
    });
    map.on("mouseleave", "cells-dot", () => {
...
function fit(map, cells, maplibregl) {
  ...
  if (cells.length === 1) {
    map.easeTo({ center: [cells[0].lon, cells[0].lat], zoom: 9 });
    return;
  }
