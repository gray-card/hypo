...
const SCRATCH =
  "/private/tmp/claude-503/-Users-awhite48-Projects-grain-editor/82a2331a-d602-43bf-8987-782018dcb4a9/scratchpad";
...
let model = { phrases: [] };
try {
  model = JSON.parse(readFileSync(resolve(ROOT, "src/data/tokenizer-model.json"), "utf8"));
} catch {
  /* placeholder */
}
...
const FIXTURE = [
  ...
  "a café au lait on a wooden table",
  "cities skyline at night with tall buildings",
  "a red double decker bus in london",
  "glasses of wine and a plate of cheese",
  "children riding bicycles in the park",
  "black and white photo of a cat",
  "a bunch of balloons at a birthday party",
  "the eiffel tower in paris france",
  "boxes of fresh vegetables at the market",
...
async function loadCorpus() {
  ...
  for await (const line of rl) {
    ...
    out.push(t >= 0 ? line.slice(t + 1) : line);
...
async function main() {
  ...
  console.log(
    `  |V| new base (stemmed):     ${vBase.size.toLocaleString()}  (stemming ${vBase.size <= vOld.size ? "reduced" : "grew"} types by ${Math.abs(vOld.size - vBase.size).toLocaleString()})`,
  );
  console.log(`  |V| new with phrases:       ${vPhrase.size.toLocaleString()}`);
  console.log(
    `  phrase-token occurrences:   ${phraseTokenOcc.toLocaleString()} (${((100 * phraseTokenOcc) / Math.max(1, tokenOcc)).toFixed(2)}% of tokens)`,
  );
  ...
  console.log(
    `  idf(most common '${dfEntries[0][0]}')=${idfHi.toFixed(3)}  idf(rarest kept)=${idfLo.toFixed(3)}  idf(OOV)=${idf("zzznotaword").toFixed(3)}`,
  );

  console.log("\nHARD INVARIANTS");
  check(
    "df fixed point: every df key re-tokenizes to itself",
    Object.keys(df).every((k) => {
      const t = phraseTok(k);
  ...
  {
    const ix = buildTextIndex([{ uri: "a", text: "a dog in a field" }]);
    check("A: 'dogs' query matches 'dog' caption (stemming)", (bm25Search(ix, tokenize("dogs")).get("a") || 0) > 0);
  }
  ...
  if (model.phrases.includes("hot_air_balloon")) {
    const docs = [
      ...
      { uri: "d3", text: "an air conditioner" },
      { uri: "d4", text: "a balloon animal" },
    ];
    const ix = buildTextIndex(docs, phraseTok);
    const s = bm25Search(ix, phraseTok("hot air balloon"));
    check(
      "B: 'hot air balloon' concentrates on the target doc",
      (s.get("d1") || 0) > 0 && ["d2", "d3", "d4"].every((d) => (s.get(d) || 0) === 0),
    );
  } else console.log("  [skip] B: phrase model missing 'hot_air_balloon' (placeholder?)");
  // Probe C: diacritic-folded query.
  {
    const ix = buildTextIndex([{ uri: "c", text: "café au lait" }]);
    check("C: 'cafe' query retrieves 'café' caption", (bm25Search(ix, tokenize("cafe")).get("c") || 0) > 0);
  }

  // Dump top phrases for human spot-judging.
  if (model.phrases.length) {
    const top = model.phrases
      .slice(0, 60)
      .map((p) => `${p.replace(new RegExp(PHRASE_JOINER, "g"), " ")}\t`)
      .join("\n");
    const out = resolve(SCRATCH, "phrases-to-judge.tsv");
    try {
      writeFileSync(out, "phrase\ttrue_MWU(y/n)\n" + top + "\n");
      console.log(`\nWrote top-60 phrases for spot-judging -> ${out}`);
    } catch {
      /* scratch not writable */
    }
  }
