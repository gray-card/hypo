...
const captionOf = (line) => {
  const t = line.indexOf("\t");
  return t >= 0 ? line.slice(t + 1) : line;
};
...
const shardOf = (key) => {
  ...
  return (h & 0x7fffffff) % NSHARDS;
};
const bump2 = (key) => {
  const m = B[shardOf(key)];
...
async function main() {
  ...
  console.error(
    `[tokenizer] gates: min2=${opts.min2} min3=${opts.min3} npmi>=${opts.npmi} maxlen=${opts.maxlen} top=${opts.top} cap=${opts.cap}`,
  );
    ...
    N = 0;
  {
    ...
    for await (const line of rl) {
      ...
      if (N % 1000000 === 0)
        console.error(
          `[tokenizer] pass1 ${N.toLocaleString()} captions, ${c1.size.toLocaleString()} unigrams, ${B.reduce((a, m) => a + m.size, 0).toLocaleString()} bigrams`,
        );
    }
  }
  if (!N) throw new Error("no captions parsed — check input format (url<TAB>caption)");
  console.error(
    `[tokenizer] pass1 done: N=${N.toLocaleString()} N1=${N1.toLocaleString()} Nb=${Nb.toLocaleString()} Nt=${Nt.toLocaleString()}`,
  );
  ...
  if (opts.maxlen >= 3 && acceptedBi.size) {
    ...
    for await (const line of rl) {
      ...
      if (N2 % 2000000 === 0)
        console.error(
          `[tokenizer] pass2 ${N2.toLocaleString()} captions, ${c3.size.toLocaleString()} candidate trigrams`,
        );
    }
    let acc3 = 0;
  ...
  const model = {
    ...
    meta: {
      ...
      params: {
        ...
        minCount3: opts.min3,
        npmi: opts.npmi,
        maxPhraseLen: achievedMax,
        requestedMaxLen: opts.maxlen,
        top: opts.top,
        cap: opts.cap,
      },
  ...
  console.error(
    `[tokenizer] wrote ${opts.out}: ${phrases.length.toLocaleString()} phrases (${bi.toLocaleString()} bi, ${(phrases.length - bi).toLocaleString()} tri+)`,
  );
  console.error(`[tokenizer] sample: ${phrases.slice(0, 12).join(", ")}`);
}

main().catch((e) => {
  console.error("[tokenizer] FAILED:", e.message);
  process.exit(1);
});
