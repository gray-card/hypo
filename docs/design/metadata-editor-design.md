...
> doc fixes _what_ each editor may collect (Derive / Reconcile / Author); this one designs _how_. The
...
and other records, with each field wearing a **provenance chip**; the user _reviews and confirms_ a
...
_stop re-checking_ prefilled values. Thus **confidence display and a direct override control are
required whenever Hypo supplies a derived value**. [nngroup.com/articles/eas-framework-simplify-forms]
...
| gear _type_ identity             | reconciled authority ID · shared network type · manual          |
...
- so the _same_ lexicon can drive the form. [atproto.com/guides/lexicon] But a schema says _what_, not
  _how to render_: every mature generator (RJSF's `uiSchema`, JSON Forms' UI schema, SHACL-UI's
...
**HERITRACE** (a SHACL-driven RDF curation tool). There is _no_ mature general lexicon-driven record
...
API** as the _internal contract_ and make each source an **adapter** behind it. [reconciliation-api.github.io/specs]
...
1. **Suggest / typeahead**: as the user types, query adapters in order: **(a) the network catalog**
   ...
   **(d) Lensfun** (for bodies/lenses). Narrow by known properties (reconcile "50mm" _as a lens_,
...
4. **Create is the fallback**: a "+ Create `input`" option appears only at the end of the candidate
   list; on save, re-run a duplicate check against the network + authorities and offer _"Did you mean
   `X`? Link instead"_ before committing. [drupal inline_entity_form search-before-create]
...
| **Getty AAT**                                            | reconcile _type/material_ vocab (processes, apparatus, chemistry)                 | **ODC-By**: attribute                                                  |
...
- Any instance field may **override** its inherited value, stored as a **delta** only (serial, shutter
  ...
  These are the _only_ fields the instance form authors.
- Because the type ref is **CID-pinned**, the instance can detect "type updated since link" and offer
  to **re-pin** without changing the reference silently. This matches ATProto's strongRef (AT-URI + CID) semantics.
...
- **`sameAs`**: use **`owl:sameAs`** _only_ for genuine identity (two repos' records that are literally
  the same camera type); overuse is a known anti-pattern. For links to an _external authority_ use
...
- **Carry provenance on every link, not a bare pointer**: SSSOM-style
  `{ predicate, confidence, source, method: human | auto }`. This is a small, worthwhile **lexicon addition** to `catalogLinks`
...
1. **Per-link provenance on `catalogLinks`**: an SSSOM-style
   `{ predicate: owl:sameAs | skos:exactMatch | skos:closeMatch, confidence, source, method }` on each `sameAs` entry, so a linked identity carries
   its trust (§6). Additive; today's bare `sameAs` becomes a degenerate case.
2. **A source enum for `fieldProvenance`** aligned to the precedence table (§2):
   `exif | bundle | lensDb | filmDb | editGraph | authority | manual`, so the chip's source is a known value, not free text.
...
- The **network catalog** discovery: how Hypo finds _other users'_ shared `catalog.*` types for
