...
The checked-in sidecar tags the first stable snapshot as `lexicons-v1`. No transition chain exists because this release establishes the baseline rather than migrating from an earlier published suite. After the corresponding `v1.0.0` Git tag is present on `main`, pull-request checks classify later schema changes against the released line and require reviewed migrations for acknowledged breaks. See [How schema versions work](../explanation/schema-versions.md) for the user-facing model and [Make a breaking lexicon change](../how-to/make-breaking-lexicon-change.md) for the release procedure.
