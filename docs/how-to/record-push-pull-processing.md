...
3. Preserve the recipe's `sourceDocument`, `sourceSpec`, and `pushPull` value.
...
`publishedTimeSeconds` means that the selected source supports that time. `actualTimeSeconds` records how long the bath ran. `pushPull` describes the intended deviation from box-speed processing; it does not establish that a particular time is authoritative.
