...
Process records separate selected source values from observed values. In a development session, `publishedTimeSeconds` and `temperatureSetpoint` retain the plan; `actualTimeSeconds` and `actualTemperature` record the run. `sourceDocument`, `sourceSpec`, recipe status, and provenance explain why the plan was chosen.
